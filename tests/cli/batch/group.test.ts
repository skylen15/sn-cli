...
describe("batch group", () => {
  it("prints the group's help and exits 0 when invoked with no leaf", () => {
    const { stdout, code } = spawn(["batch"]);
    assert.equal(code, 0);
    assert.match(stdout, /USAGE/);
    assert.match(stdout, /sn batch/);
  });

  it("lists the group's leaves in group help and the group in root help", () => {
    const group = spawn(["batch", "--help"]);
    ...
    assert.match(root.stdout, /\bbatch\b/);
  });
  ...
  it("exits non-zero with an explanation for each retired flat name", () => {
    for (const name of ["batch-update", "batch-delete"] as const) {
