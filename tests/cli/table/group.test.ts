...
describe("table group", () => {
  it("prints the group's help and exits 0 when invoked with no leaf", () => {
    const { stdout, code } = spawn(["table"]);
    assert.equal(code, 0);
    assert.match(stdout, /USAGE/);
    assert.match(stdout, /sn table/);
  });

  it("lists the group's leaves in group help and the group in root help", () => {
    const group = spawn(["table", "--help"]);
    assert.equal(group.code, 0);
    assert.match(group.stdout, /\bquery\b/);
    assert.match(group.stdout, /\bschema\b/);
    assert.match(group.stdout, /\bconfig\b/);
